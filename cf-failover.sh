...
if [[ -z "$DESIRED_IP" ]]; then
  ...
  notify_telegram "🚨 *CF-Failover ONYO*: Node ZOTE (${NODE_PRIORITY}) hazirespondi. Hakuna DNS iliyobadilishwa - tafadhali angalia servers zako."
  save_state
  exit 0
fi
...
for record in "${RECORDS_ARR[@]}"; do
  ...
  if update_dns "$record" "$record_id" "$DESIRED_IP"; then
    notify_telegram "🔁 *CF-Failover*: \`${record}\` imebadilishwa: \`${current_ip:-haipo}\` ➜ \`${DESIRED_IP}\`"
  fi
done
